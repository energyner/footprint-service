...
document.getElementById("calcular").addEventListener("click", async function (event) {
    ...
    if (hostname === "localhost" || hostname === "127.0.0.1") {
      ...
      console.log(
        "Entorno detectado: Desarrollo local (laptop). URL API Footprint:",
      ...
      console.log(
        "Entorno detectado: Desarrollo local (LAN). URL API Footprint:",
        API_FOOTPRINT_FINAL_URL
      );
    } else {
      ...
      console.log(
        "Entorno detectado: Producción (Cloud Run). URL API Footprint:",
        API_FOOTPRINT_FINAL_URL
      );
    }
    ...
    try {
      ...
      document.getElementById("per_capita_estado").value =
        result.per_capita_estado || 0;
      document.getElementById("promedioUSA").value = result.promedioUSA || 0;
      document.getElementById("promedioMundial").value =
        result.promedioMundial || 0;
      document.getElementById("porcentajeEstado").value =
        (result.porcentajeEstado || 0) + "%";
      document.getElementById("porcentajeUSA").value =
        (result.porcentajeUSA || 0) + "%";
      document.getElementById("porcentajeMundial").value =
        (result.porcentajeMundial || 0) + "%";
