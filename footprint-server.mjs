...
app.get("/health", (req, res) => {
  ...
  if (typeof calcularHuellaCarbono === "function") {
    ...
    try {
      ...
      if (typeof testResult === "object" && !testResult.error) {
        // Verifica que no haya error en el resultado
        isCalculationFunctionWorking = true;
      }
    } catch (error) {
      console.error(
        "Error al ejecutar calcularHuellaCarbono en health check:",
        error
      );
    }
  }
  ...
  if (isServerUp && isCalculationFilePresent && isCalculationFunctionWorking) {
    ...
    if (!isCalculationFunctionWorking)
      message += " La función no se ejecuta correctamente o devuelve un error.";
...
app.post("/api/huella-carbono", (req, res) => {
  ...
  const result = calcularHuellaCarbono({
    state,
    elect,
    gas,
    water,
